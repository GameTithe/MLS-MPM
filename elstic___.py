...
particles = Particle.field(shape=(64, 64))
visual_pos = ti.Vector.field(3, dtype=ti.f32, shape= particles.shape[0] * particles.shape[1])
...
def G2P():
    for i, j in particles:
        ...
        particles[i, j].pos = particle.pos
...
def visualization():
    for i, j in particles:
        visual_pos[i * particles.shape[0] + j] = particles[i, j].pos
